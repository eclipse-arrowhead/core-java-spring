...
\ArrowheadTitle{orchestration-qos-confirm-reservation HTTP/TLS/JSON} %e.g. ServiceDiscovery HTTP/TLS/JSON
\ArrowheadServiceID{orchestration-qos-confirm-reservation} % e.g. register
...
The rest of this document describes how to realize the \textit{orchestration-qos-confirm-reservation} service HTTP/TLS/JSON interface in details.
...
\begin{lstlisting}[language=http,label={lst:orchestration-qos-confirm-reservation},caption={An \fref{orchestration-qos-confirm-reservation} invocation.}]
POST /orchestrator/qos_reservation HTTP/1.1
...
orList & \pref{List}$<$\hyperref[sec:model:OrchestrationResult]{OrchestrationResult}$>$ & yes & A result list of an orchestration request. The providers in the list are the subject of the lock. \\ \hline
