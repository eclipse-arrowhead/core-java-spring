...
\ArrowheadTitle{orchestration-service HTTP/TLS/JSON} %e.g. ServiceDiscovery HTTP/TLS/JSON
\ArrowheadServiceID{orchestration-service} % e.g. register
...
\begin{table}[ht!]
  ...
  \end{tabular}
  \caption{Communication and sematics details used for the \textbf{orchestration-service}
    service interface}
...
The rest of this document describes how to realize the \textit{orchestration-service} service HTTP/TLS/JSON interface in details.
...
The service responses with the status code \texttt{200 Ok} if called successfully. The error codes are, \texttt{400 Bad Request} if request is malformed, \texttt{401 Unauthorized} if improper client side certificate is provided, \texttt{500 Internal Server Error} if Service Registry is unavailable.
