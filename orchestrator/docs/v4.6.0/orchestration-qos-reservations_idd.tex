...
The rest of this document describes how to realize the orchestration-qos-reservations service HTTP/TLS/JSON interface in details.
...
The service responses with the status code \texttt{201 Created} if called successfully. The error codes are, \texttt{401 Unauthorized} if improper client side certificate is provided, \texttt{500 Internal Server Error} if Orchestrator is unavailable.
