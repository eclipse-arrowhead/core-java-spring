...
INSERT INTO `msvc_mip_category` (`name`, `abbreviation`)
...
INSERT INTO `msvc_mip_domain` (`name`)
VALUES ('SAFETY'),
       ('SECURITY');

INSERT INTO `msvc_standard` (`identification`, `name`, `description`, `reference_uri`)
...
INSERT INTO `msvc_mip` (`ext_id`, `name`, `description`, `category_id`, `standard_id`, `domain_id`)
VALUES (1, 'Human user identification and authentication',
        ...
        (SELECT id FROM msvc_mip_category WHERE abbreviation = 'IAC'),
        (SELECT id FROM msvc_standard WHERE identification = 'IEC 62443-3-3'),
        (SELECT id FROM msvc_mip_domain WHERE name = 'SECURITY'));

INSERT INTO `msvc_mip_verification_list` (`name`, `description`)
VALUES ('Default verification list', null);

INSERT INTO `msvc_mip_verification_entry` (`mip_id`, `weight`, `verification_list_id`)
VALUES ((SELECT id FROM msvc_mip WHERE name = 'Human user identification and authentication'),
        100,
        (SELECT id FROM msvc_mip_verification_list WHERE name = 'Default verification list'));
