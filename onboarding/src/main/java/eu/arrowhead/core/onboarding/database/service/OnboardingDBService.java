...
public class OnboardingDBService {
    ...
    @Autowired
    public OnboardingDBService(final OrchestrationDriver orchestration, HttpService httpService) {
    ...
    //-------------------------------------------------------------------------------------------------
    private OnboardingWithCsrResponseDTO executeCertificateSigningRequest(final String certificateSigningRequest) {
        ...
        retValue.setRootCertificate(csrResult.getCertificateChain().get(0));
        retValue.setIntermediateCertificate(csrResult.getCertificateChain().get(1));
        retValue.setOnboardingCertificate(certificateResponseDTO);
        ...
        retValue.setDeviceRegistry(new ServiceEndpoint(CoreSystemService.DEVICE_REGISTRY_REGISTER_SERVICE, orchestration.createUri(drOrchResult).toUri()));
        retValue.setSystemRegistry(new ServiceEndpoint(CoreSystemService.SYSTEM_REGISTRY_REGISTER_SERVICE, orchestration.createUri(sysrOrchResult).toUri()));
        retValue.setServiceRegistry(new ServiceEndpoint(CoreSystemService.SERVICE_REGISTRY_REGISTER_SERVICE, orchestration.createUri(srOrchResult).toUri()));
        retValue.setOrchestrationService(new ServiceEndpoint(CoreSystemService.ORCHESTRATION_SERVICE, orchestration.createUri(orchOrchResult).toUri()));
    ...
    private void pingService(final SystemResponseDTO system, final String uriPrefix) {
        ...
        final UriComponents echoUri = orchestration.createUri(system, uriPrefix + CommonConstants.ECHO_URI);
