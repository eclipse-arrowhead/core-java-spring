...
public class ConfigurationControllerLogsTest {
	...
	private static final String LOGS_URL = "/configuration/mgmt/logs";
	...
	public void testGetLogEntriesWithoutParameter() throws Exception  {
		final Logs entry = new Logs("log", ZonedDateTime.now(), "logger", CoreSystem.CONFIGURATION, LogLevel.INFO, "test", null);
		final Page<Logs> page = new PageImpl<>(List.of(entry));

		when(commonDBService.getLogEntriesResponse(anyInt(), anyInt(), any(Direction.class), anyString(), eq(CoreSystem.CONFIGURATION), isNull(), isNull(), isNull(), isNull())).thenReturn(DTOConverter.convertLogsPageToLogEntryListResponseDTO(page));
		...
		verify(commonDBService, times(1)).getLogEntriesResponse(0, Integer.MAX_VALUE, Direction.ASC, "logId", CoreSystem.CONFIGURATION, null, null, null, null);
	}
	...
	public void testGetLogEntriesWithAllParameters() throws Exception  {
		...
		final Logs entry = new Logs("log", from.plusSeconds(10), "logger", CoreSystem.CONFIGURATION, LogLevel.INFO, "test", null);
		final Page<Logs> page = new PageImpl<>(List.of(entry));

		when(commonDBService.getLogEntriesResponse(anyInt(), anyInt(), any(Direction.class), anyString(), eq(CoreSystem.CONFIGURATION), anyList(), any(ZonedDateTime.class), any(ZonedDateTime.class), anyString())).thenReturn(DTOConverter.convertLogsPageToLogEntryListResponseDTO(page));
		...
		verify(commonDBService, times(1)).getLogEntriesResponse(0, 5, Direction.DESC, "entryDate", CoreSystem.CONFIGURATION, logLevels, from, to, "logger");
	}
}