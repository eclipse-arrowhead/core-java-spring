...
\begin{itemize}
    \item It must contain the consumer system's certificate common name, the service definition an the interface name
    \item It could contain a duration time.
...
This section describes the interfaces to the service. The \textbf{token-generation} service is used to access tokens. The various parameters are representing the necessary system and service input information.
...
\fsubsection{HTTP/TLS/JSON}{TokenGenerationRequest}{TokenGenerationResponse}
...
tokens &  \pref{Map}$<$\pref{Interface},\pref{String}$>$ & Interface-token pairs \\ \hline 
