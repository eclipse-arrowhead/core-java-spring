...
\ArrowheadTitle{authorization-control-subscription} % XXX = ServiceName 
\ArrowheadServiceID{authorization-control-subscription} % ID name of service
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{authorization-control-subscription} service. 
  \end{abstract}
...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{authorization-control-subscription} service, which enables publish-subscribe type authorization control within a local cloud. The purpose of this service is to grant access right for a consumer to being subscribed for the events published by a specified provider.
...
This section describes the interfaces to the service. The \textbf{authorization-control-subscription} service is used to verify authorization rules. The various parameters are representing the necessary system and service input information.
...
\fsubsection{HTTP/TLS/JSON}{CheckAuthSubscriptionRequest}{CheckAuthSubscriptionResponse}
...
Here, all data objects that can be part of the \textbf{authorization-control-subscription} service
...
publishers & \pref{List}$<$\hyperref[sec:model:SystemRecord]{SystemRecord}$>$ & yes & Descriptor of the authorized publisher systems. \\ \hline
\end{tabularx}
\end{table}
