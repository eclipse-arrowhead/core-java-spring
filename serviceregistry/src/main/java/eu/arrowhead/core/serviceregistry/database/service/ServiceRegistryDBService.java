...
public class ServiceRegistryDBService {
    ...
    //-------------------------------------------------------------------------------------------------
    public ServiceDefinitionsListResponseDTO getServiceDefinitionEntriesResponse(final int page, final int size, final Direction direction,
    ...
    public ServiceRegistryGroupedResponseDTO getServiceRegistryDataForServiceRegistryGroupedResponse() {
        ...
        try {
            ...
            return DTOConverter.convertServiceRegistryDataToServiceRegistryGroupedResponseDTO(serviceDefinitionEntries, systemEntries, interfaceEntries,
    ...
    //-------------------------------------------------------------------------------------------------
    public Page<ServiceRegistry> getServiceRegistryEntriesByServiceDefinition(final String serviceDefinition, final int page, final int size,
        ...
        try {
            final Optional<ServiceDefinition> serviceDefinitionOptional = serviceDefinitionRepository
                    .findByServiceDefinition(serviceDefinition.toLowerCase().trim());
            if (serviceDefinitionOptional.isPresent()) {
                return serviceRegistryRepository.findAllByServiceDefinition(serviceDefinitionOptional.get(), PageRequest
                        .of(validatedPage, validatedSize, validatedDirection, validatedSortField));
            } else {
    ...
    //-------------------------------------------------------------------------------------------------
    public ServiceRegistryListResponseDTO getServiceRegistryEntriesByServiceDefinitionResponse(final String serviceDefinition, final int page, final int size,
                                                                                               final Direction direction,
                                                                                               final String sortField) {
        logger.debug("getServiceRegistryEntriesByServiceDefinitionResponse started...");

        final Page<ServiceRegistry> serviceRegistryEntriesByServiceDefinition = getServiceRegistryEntriesByServiceDefinition(serviceDefinition, page, size,
                                                                                                                             direction, sortField);
    ...
    public ServiceRegistryResponseDTO registerServiceResponse(final ServiceRegistryRequestDTO request) {
        ...
        try {
            ...
            final System provider = findOrCreateSystem(validatedProviderName, validatedProviderAddress, validatedProviderPort,
                                                       request.getProviderSystem().getAuthenticationInfo());

            final ZonedDateTime endOfValidity = Utilities.isEmpty(request.getEndOfValidity()) ? null : Utilities
                    .parseUTCStringToLocalZonedDateTime(request.getEndOfValidity().trim());
            ...
            final ServiceRegistry srEntry = createServiceRegistry(serviceDefinition, provider, request.getServiceUri(), endOfValidity, validatedSecurityType,
                                                                  metadataStr, version,
    ...
    public ServiceRegistryResponseDTO updateServiceByIdResponse(final long id, final ServiceRegistryRequestDTO request) {
        ...
        try {
            ...
            final String metadataStr = Utilities.map2Text(request.getMetadata());
            final int version = request.getVersion() == null ? 1 : request.getVersion().intValue();
            srEntry = updateServiceRegistry(srEntry, serviceDefinition, provider, request.getServiceUri(), endOfValidity, validatedSecurityType, metadataStr,
                                            version, request.getInterfaces());
    ...
    public ServiceRegistryResponseDTO mergeServiceByIdResponse(final long id, final ServiceRegistryRequestDTO request) {
        ...
        try {
            ...
            final int validatedProviderPort = (request.getProviderSystem() != null && request.getProviderSystem().getPort() != null) ? request
                    .getProviderSystem().getPort().intValue() :
            ...
            final ServiceDefinition serviceDefinition = optServiceDefinition.isPresent() ? optServiceDefinition.get() : createServiceDefinition(
                    validatedServiceDefinition);

            System provider;
            if (request.getProviderSystem() != null) {
                final Optional<System> optProvider = systemRepository
                        .findBySystemNameAndAddressAndPort(validatedProviderName, validatedProviderAddress, validatedProviderPort);
                if (optProvider.isPresent()) {
                    provider = optProvider.get();
                    if (!Objects
                            .equals(request.getProviderSystem().getAuthenticationInfo(), provider.getAuthenticationInfo())) { // authentication info has changed
                    ...
                    provider = createSystem(validatedProviderName, validatedProviderAddress, validatedProviderPort,
                                            request.getProviderSystem().getAuthenticationInfo());
                }
            } else {
            ...
            final List<String> validatedInterfaces = request.getInterfaces() != null && !request.getInterfaces().isEmpty() ? request
                    .getInterfaces() : validatedInterfacesTemp;

            srEntry = updateServiceRegistry(srEntry, serviceDefinition, provider, validatedServiceUri, endOfValidity, validatedSecurityType,
                                            validatedMetadataStr, validatedVersion,
                                            validatedInterfaces);
    ...
    //-------------------------------------------------------------------------------------------------
    @Transactional(rollbackFor = ArrowheadException.class)
    public ServiceRegistry createServiceRegistry(final ServiceDefinition serviceDefinition, final System provider, final String serviceUri,
                                                 final ZonedDateTime endOfValidity,
                                                 final ServiceSecurityType securityType, final String metadataStr, final int version,
        ...
        try {
            ...
            final ServiceRegistry srEntry = serviceRegistryRepository
                    .save(new ServiceRegistry(serviceDefinition, provider, validatedServiceUri, endOfValidity, secure, metadataStr, version));
    ...
    //-------------------------------------------------------------------------------------------------
    @Transactional(rollbackFor = ArrowheadException.class)
    public ServiceRegistry updateServiceRegistry(final ServiceRegistry srEntry, final ServiceDefinition serviceDefinition, final System provider,
                                                 final String serviceUri,
                                                 final ZonedDateTime endOfValidity, final ServiceSecurityType securityType, final String metadataStr,
                                                 final int version,
                                                 final List<String> interfaces) {
        ...
        return setModifiedValuesOfServiceRegistryEntryFields(srEntry, serviceDefinition, provider, serviceUri, endOfValidity, securityType, metadataStr,
                                                             version, interfaces);
    }
    ...
    @Transactional(rollbackFor = ArrowheadException.class)
    public void removeServiceRegistry(final String serviceDefinition, final String providerSystemName, final String providerSystemAddress,
                                      final int providerSystemPort) {
        ...
        try {
            ...
            final Optional<System> optProviderSystem = systemRepository
                    .findBySystemNameAndAddressAndPort(validatedSystemName, validatedSystemAddress, providerSystemPort);
            if (optProviderSystem.isEmpty()) {
                throw new InvalidParameterException(
                        "No system with name: " + validatedSystemName + ", address: " + validatedSystemAddress + ", port: " + providerSystemPort +
                                " exists.");
            }

            final Optional<ServiceRegistry> optServiceRegistryEntry = serviceRegistryRepository
                    .findByServiceDefinitionAndSystem(optServiceDefinition.get(), optProviderSystem.get());
            if (optServiceRegistryEntry.isEmpty()) {
                throw new InvalidParameterException(
                        "No Service Registry entry with provider: (" + validatedSystemName + ", " + validatedSystemAddress + ":" + providerSystemPort +
    ...
    // assistant methods
    //-------------------------------------------------------------------------------------------------
    private void sendEvent(final String eventType, final ServiceRegistry serviceRegistry) {
        try {
            // we don't send events for core systems coming online
            final var systemName = serviceRegistry.getSystem().getSystemName();
            for (CoreSystem coreSystem : CoreSystem.values()) {
                if(coreSystem.name().equalsIgnoreCase(systemName)) return;
            }

            final var requestDTO = convertRequest(serviceRegistry);
            final var srEntries = getServiceRegistryEntriesByServiceDefinition(CoreSystemService.EVENT_PUBLISH_SERVICE.getServiceDefinition(),
                                                                               0, 0, null, null);

            // single try weak effort
            final var anyEventHandler = srEntries.get().findAny();
            if (anyEventHandler.isEmpty()) {
                return;
            }

            final var uri = UriComponentsBuilder.fromUriString(anyEventHandler.get().getServiceUri()).build();
            final var request = new EventPublishRequestDTO(eventType,
            ...
            logger.warn("Unable to publish {} event: {}", eventType, e.getMessage());
        }
    }

    //-------------------------------------------------------------------------------------------------
    private void publishRegister(final ServiceRegistry serviceRegistry) {
        sendEvent(CoreEventHandlerConstants.REGISTER_SERVICE_EVENT, serviceRegistry);
    }

	//-------------------------------------------------------------------------------------------------
	private void publishUnregister(final ServiceRegistry serviceRegistry) {
		sendEvent(CoreEventHandlerConstants.UNREGISTER_SERVICE_EVENT, serviceRegistry);
	}
    ...
    private void checkSRSecurityValue(final ServiceSecurityType type, final String providerSystemAuthenticationInfo) {
        ...
        Assert.isTrue(
                validatedType == ServiceSecurityType.NOT_SECURE || (validatedType != ServiceSecurityType.NOT_SECURE && providerSystemAuthenticationInfo != null),
    ...
    private void checkConstraintOfSystemRegistryTable(final ServiceDefinition serviceDefinition, final System provider) {
        ...
        try {
            ...
            if (find.isPresent()) {
                throw new InvalidParameterException(
                        "Service Registry entry with provider: (" + provider.getSystemName() + ", " + provider.getAddress() + ":" + provider.getPort() +
    ...
    //-------------------------------------------------------------------------------------------------
    private boolean checkServiceRegistryIfUniqueValidationNeeded(final ServiceRegistry srEntry, final ServiceDefinition serviceDefinition,
                                                                 final System provider) {
    ...
    //-------------------------------------------------------------------------------------------------
    private ServiceRegistry setModifiedValuesOfServiceRegistryEntryFields(final ServiceRegistry srEntry, final ServiceDefinition serviceDefinition,
                                                                          final System provider,
                                                                          final String serviceUri, final ZonedDateTime endOfValidity,
                                                                          final ServiceSecurityType securityType,
