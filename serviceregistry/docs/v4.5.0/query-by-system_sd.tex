...
\ArrowheadTitle{query-by-system} % XXX = ServiceName 
\ArrowheadServiceID{query-by-system} % ID name of service
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{query-by-system} service. 
  \end{abstract}
...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{query-by-system} service, which enables the systems to query for specific system details based on system parameters. Examples of this interaction is a dedicated core system that needs the available information for a specific system.
...
Available only for the following core systems: \textit{ Orchestrator, Choreographer}
...
This section describes the interfaces to the service. The \textbf{query-by-system} service is used to looking for a specific system. The various parameters are representing the necessary system input information.
...
Here, all data objects that can be part of the \textbf{query-by-system} service
