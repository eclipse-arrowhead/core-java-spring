...
\ArrowheadTitle{query-by-system-id} % XXX = ServiceName 
\ArrowheadServiceID{query-by-system-id} % ID name of service
\ArrowheadType{Service Description}
\ArrowheadTypeShort{SD}
\ArrowheadVersion{4.4.0} % Arrowhead version X.Y.Z, e..g. 4.4.1
\ArrowheadDate{\today}
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{query-by-system-id} service. 
  \end{abstract}
...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{query-by-system-id} service, which enables the systems to query for specific system details based on its record id. Examples of this interaction is a dedicated core system that needs the available information for a specific system.
...
The given core system is required to submit the system record id.
...
Available only for the following core systems: \textit{Orchestrator}
...
This section describes the interfaces to the service. The \textbf{query-by-system-id} service is used to looking for a specific system. In the following, each subsection names an interface, an input type and an output type, in that order.
...
\fsubsection{HTTP/TLS/JSON}{Number}{SystemResponse}
...
Here, all data objects that can be part of the \textbf{query-by-system-id} service
