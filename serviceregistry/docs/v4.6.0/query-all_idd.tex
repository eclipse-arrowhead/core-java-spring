...
\begin{table}[ht!]
  ...
  \end{tabular}
  \caption{Communication and semantics details used for the \textbf{query-all}
    service interface}
...
The rest of this document describes how to realize the \textbf{query-all} service HTTP/TLS/JSON interface in details.
...
The service responses with the status code \texttt{200
  Ok} if called successfully. The error codes are, \texttt{401 Unauthorized} if
...
data & \pref{List}$<$\hyperref[sec:model:ServiceRegistryResponse]{ServiceRegistryResponse}$>$     & List of service instances. \\ \hline
