...
\ArrowheadTitle{register-system} % XXX = ServiceName 
\ArrowheadServiceID{register-system} % ID name of service
...
\begin{center}
  ...
  % Front Page Abstract
  \begin{abstract}
    This document provides service description for the \textbf{register-system} service. 
  \end{abstract}
...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{register-system} service, which enables autonomous system registration, therefore it is an integral part of the implementation of service discovery requirements in Service Registry Mandatory Core System. Example of this interaction is a system that has the purpose of consuming some kind of service. To enable the system being authorized to consume, the system must be a registered member of the local cloud.
...
The given service consumer application system is required to use the  \textbf{register-system} service at its startup or at least before looking for services.
...
Available for anyone within the local cloud, but in case of secure mode the system is allowed to register only 
...
This section describes the interfaces to the service. The \textbf{register-system} service is used to register system. A system could contain various metadata as well as a physical address. The various parameters are representing the necessary system input information.
...
\fsubsection{HTTP/TLS/JSON}{SystemRequest}{SystemResponse}
...
Here, all data objects that can be part of the \textbf{register-system} service
