...
\ArrowheadTitle{query-multi HTTP/TLS/JSON} %e.g. ServiceDiscovery HTTP/TLS/JSON
\ArrowheadServiceID{query-multi} % e.g. register
...
\begin{table}[ht!]
  ...
  \end{tabular}
  \caption{Communication and sematics details used for the \textbf{query-multi}
    service interface}
  \label{tab:comunication_semantics_profile}
\end{table}

This document provides the Interface Design Description IDD to the \textit{query-multi -- Service Description} document.
For further details about how this service is meant to be used, please consult that document.

The rest of this document describes how to realize the query-multi service HTTP/TLS/JSON interface in details.
...
\begin{lstlisting}[language=http,label={lst:register},caption={A \fref{query-multi} invocation.}]
POST /serviceregistry/query/multi HTTP/1.1
...
\begin{lstlisting}[language=http,label={lst:register_response},caption={A \fref{query-multi} response.}]
{
  "results": [
    {
     "serviceQueryData": [
...
maxVersionRequirement &\pref{Number} & no & Required maximum version of the ser\-vice. If specified version must be equals or lower or having result(s). Ignored if \textit{versionRequirement} is specified. \\ \hline
...
minVersionRequirement &\pref{Number} & no & Required minimum version of the ser\-vice. If specified version must be equals or higher or having result(s). Ignored if \textit{versionRequirement} is specified. \\ \hline
...
serviceDefinitionResponse & \hyperref[sec:model:ServiceDefinitionRecord]{ServiceDefinitionRecord} & Descriptor of the serviceDefinition record. \\ \hline
