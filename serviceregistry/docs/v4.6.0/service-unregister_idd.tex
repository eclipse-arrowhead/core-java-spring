...
\ArrowheadTitle{service-unregister HTTP/TLS/URL} %e.g. ServiceDiscovery HTTP/TLS/JSON
...
%  \Large{Service ID: \textit{"\arrowid"}}
  ...
  \begin{abstract}
    ...
    security and URL encoding variant of the \textbf{service-unregister} service.
  \end{abstract}
  \vspace*{1cm}
...
\begin{table}[ht!]
  ...
  \end{tabular}
  \caption{Communication and semantics details used for the \textbf{service-unregister}
    service interface}
...
The rest of this document describes how to realize the \textbf{service-unregister} service HTTP/TLS/URL interface in details.
...
With these primitives now available, we proceed to define all the types specified in the \textbf{service-uregister} SD primitives either as \textit{aliases} or \textit{structs}.
