...
\section{Overview}
\label{sec:overview}
This document describes the \textbf{query} service, which enables autonomous service look up, therefore it is an integral part of the implementation of service discovery requirements in Service Registry Mandatory Core System. Example of this interaction is a system that has the capability to consume some kind of public core service. In order the system will be aware, where and how the service could be consumed, it needs to query the service details from Service Registry Core System.
...
The given system is required to being aware how to consume the \textbf{query} service from Service Registry Core System in order to look for the other public core services such as \textbf{orchestration-service} from the Orchestrator Core System. Figure \ref{fig:activity_uml} describes the processing of query data.

\begin{figure}[h!]
  \centering
  \includegraphics[width=6.3cm]
