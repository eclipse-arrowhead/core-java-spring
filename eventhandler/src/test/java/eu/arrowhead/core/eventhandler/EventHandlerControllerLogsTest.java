...
public class EventHandlerControllerLogsTest {
	...
	private static final String LOGS_URL = "/eventhandler/mgmt/logs";
	...
	public void testGetLogEntriesWithoutParameter() throws Exception  {
		final Logs entry = new Logs("log", ZonedDateTime.now(), "logger", CoreSystem.EVENTHANDLER, LogLevel.INFO, "test", null);
		final Page<Logs> page = new PageImpl<>(List.of(entry));

		when(commonDBService.getLogEntriesResponse(anyInt(), anyInt(), any(Direction.class), anyString(), eq(CoreSystem.EVENTHANDLER), isNull(), isNull(), isNull(), isNull())).thenReturn(DTOConverter.convertLogsPageToLogEntryListResponseDTO(page));
		...
		verify(commonDBService, times(1)).getLogEntriesResponse(0, Integer.MAX_VALUE, Direction.ASC, "logId", CoreSystem.EVENTHANDLER, null, null, null, null);
	}
	...
	public void testGetLogEntriesWithAllParameters() throws Exception  {
		...
		when(commonDBService.getLogEntriesResponse(anyInt(), anyInt(), any(Direction.class), anyString(), eq(CoreSystem.EVENTHANDLER), anyList(), any(ZonedDateTime.class), any(ZonedDateTime.class), anyString())).thenReturn(DTOConverter.convertLogsPageToLogEntryListResponseDTO(page));
		...
		verify(commonDBService, times(1)).getLogEntriesResponse(0, 5, Direction.DESC, "entryDate", CoreSystem.EVENTHANDLER, logLevels, from, to, "logger");
	}
}